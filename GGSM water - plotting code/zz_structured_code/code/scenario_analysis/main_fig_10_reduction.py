...
def f_annotate_threshold_crossing_local(
                                    threshold, df_local_plot, 
                                    xytext_loc, ax, 
                                    local_facecolor="black", local_ymax=100
                                    ):
...
def f_plot_figure(upto_year=2120, sup_title="Base case"):
    ...
    for counter_dir, z_dir_case in enumerate(zip(l_directory_path, l_cases)):
        ...
        for j, x in enumerate(zip(l_file_names_ava, axes.ravel())):
            ...
            dct_local['Last timestep '+case_name.replace("\n", " ")] = round(df_local_plot.iloc[-1, 0])
...
#             ax.set_ylim([-5, 155])
            ...
            # Finding year zero
            if group_name == 'Africa':
                if counter_dir == 1:
                    f_annotate_threshold_crossing_local(
                        ax=ax, xytext_loc=(0.8, 0.8), threshold=100, df_local_plot=df_local_plot
                    )
                if counter_dir == 0:
                    f_annotate_threshold_crossing_local(
                        ax=ax, xytext_loc=(0.4, 0.8), threshold=100, df_local_plot=df_local_plot
                    )
    ...
    file_name = 'main_fig_10_reduction.png'
